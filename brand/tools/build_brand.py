...
# An <img> pointing at an SVG is still an image: the browser rasterises it into
# a bitmap, whose resolution depends on the intrinsic size, the device ratio and
# whatever compositing the ancestors force. Inline SVG is drawn as live vector
# and none of that applies. These partials are what the sites actually render.
INLINE = [
    ('declare-icon-light', 'logo-icon.html', 'brandicon', 'DeCLaRe Lab'),
    ('declare-horizontal-light', 'logo-horizontal.html', 'brandwordmark', 'DeCLaRe Lab'),
]
...
def inline_partial(name, slug, title):
    ...
    # The sites own no CSS: declare-core does. An inlined <style> block would be
    # a stylesheet living in a consumer, which the ownership audit rejects and
    # which would fork the styling anyway. Nothing here needs it: every element
    # already carries its own fill, and the ink rides currentColor.
    svg = re.sub(r'\s*<style[^>]*>.*?</style>', '', svg, flags=re.S)
    leftover = set(re.findall(r'class="([^"{]+)"', svg))
    assert not leftover, 'inline partial still references classes: %s' % leftover
    svg = svg[svg.index('<svg'):]
    svg = svg.replace('<svg', '<svg class="brand-mark {{ include.class }}" aria-hidden="{{ include.hidden | default: \'false\' }}"', 1)
    return ('{% comment %}Generated by brand/tools/build_brand.py. Do not edit.'
            '{% endcomment %}\n' + svg)


def build_includes():
    for site, cfg in SITES:
        out_dir = os.path.join(GITHUB, site, '_includes', 'brand')
        os.makedirs(out_dir, exist_ok=True)
        for name, fname, slug, title in INLINE:
            if site != 'declare-lab.github.io' and fname == 'logo-horizontal.html':
                continue
            body = inline_partial(name, slug, title)
            open(os.path.join(out_dir, fname), 'w').write(body)
            print('   %-34s %d bytes' % ('_includes/brand/' + fname, len(body)))
